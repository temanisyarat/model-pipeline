...
def evaluate_model(model, test_ds, label_encoder):
    ...
    eval_results = {
        ...
        "f1_weighted": float(
            f1_score(all_true, all_preds, average="weighted", zero_division=0)
        ),
        "precision": float(
            precision_score(all_true, all_preds, average="macro", zero_division=0)
        ),
        "recall": float(
            recall_score(all_true, all_preds, average="macro", zero_division=0)
        ),
    ...
    print(
        f"Test Accuracy: {eval_results['accuracy']:.4f}  F1-macro: {
            eval_results['f1_macro']:.4f}"
    )
    return eval_results
...
def save_evaluation_results(eval_results, output_dir):
    ...
    eval_summary = {
        ...
        "per_class": {
            cls: eval_results["class_report"][cls]
            for cls in eval_results["class_names"]
        },
